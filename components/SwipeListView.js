...
class SwipeListView extends PureComponent {
    ...
    onRowOpen(id, toValue) {
        ...
        this.openCellKey = id;
        this.props.onRowOpen && this.props.onRowOpen(id, this._rows, toValue);
    }
    ...
    renderCell(VisibleComponent, HiddenComponent, id, item, shouldPreviewRow) {
        if (!HiddenComponent) {
            return React.cloneElement(VisibleComponent, {
                ...VisibleComponent.props,
                ref: row => (this._rows[id] = row),
                onRowOpen: toValue => this.onRowOpen(id, toValue),
                onRowDidOpen: toValue =>
                    this.props.onRowDidOpen &&
                    this.props.onRowDidOpen(id, this._rows, toValue),
                onRowClose: () =>
                    this.props.onRowClose &&
                    this.props.onRowClose(id, this._rows),
                onRowDidClose: () =>
                    this.props.onRowDidClose &&
                    this.props.onRowDidClose(id, this._rows),
    ...
    renderItem(rowData, rowMap) {
        ...
        let { id } = item;
        if (!id && this.props.keyExtractor) {
            id = this.props.keyExtractor(item, index);
        }

        const shouldPreviewRow =
            typeof id !== 'undefined' && this.props.previewRowKey === id;
...
SwipeListView.propTypes = {
    ...
    /**
     * Should the row with this id do a slide out preview to show that the list is swipeable
     */
